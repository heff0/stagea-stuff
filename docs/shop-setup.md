...
### 3.1 Backend prerequisite

`shop/` is a frontend only — it requires a Saleor GraphQL API to talk to. Two supported options:

- **Local `saleor-platform`** (recommended for development): a separate compose stack from `github.com/saleor/saleor-platform` that runs Saleor 3.x with seed data. Expose it on `http://localhost:8000/graphql/`:

  ```/dev/null/saleor-platform.sh#L1-3
  git clone https://github.com/saleor/saleor-platform.git
  cd saleor-platform
  docker compose up -d
  ```

  Confirm it's healthy: `curl -s -X POST http://localhost:8000/graphql/ -H 'Content-Type: application/json' -d '{"query":"{ shop { name } }"}'` should return `{"data":{"shop":{"name":"Saleor e-commerce"}}}`.

- **Cloud Saleor** at `https://<your-instance>.saleor.cloud/graphql/`. Use the URL exactly as it appears in the dashboard, including the trailing slash.

### 3.2 Storefront dev server
...
- The dev-server log shows `POST http://<your-saleor>/graphql/ 200` requests on each page load — confirms the storefront is querying the configured backend.
- `curl -fsSL http://localhost:3000/` returns HTTP 200 with `<title>ACME Storefront, powered by Saleor &amp; Next.js | Saleor Store</title>`.
- The root path 307-redirects to `/<NEXT_PUBLIC_DEFAULT_CHANNEL>`.
...
| `NEXT_PUBLIC_DEFAULT_CHANNEL` | yes | Channel slug from Saleor Dashboard → Configuration → Channels. The upstream `saleor-platform` seeds a channel with slug `default-channel`. |
